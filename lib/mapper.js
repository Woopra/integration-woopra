...
exports.identify = function(identify){
  ...
  identify = unixifyAllDates(identify);
  traits = prefixKeys(identify.traits(), 'cv_');
...
exports.track = function(track){
  ...
  track = unixifyAllDates(track);
  props = prefixKeys(stringifyNested(track.properties()), 'ce_');
...
function cookie(message){
  return crypto
    .createHash('md5')
    .update(message.userId() || message.anonymousId())
    .digest('hex');
}
...
/**
 * Change any dates to Unix for Woopra system
 * NB: alters values in place and returns same object
 *
 * @param {Object} segmentProps from `track.properties()` or `identify.properties()`
 * @return {Object} with woopra-friendly dates
 */

function unixifyAllDates(segmentObject){

  Object.keys(segmentObject).forEach(function (key) {
    var value = segmentObject[key];

    //change iso dates to unix
    if (is.date(value)) {
      segmentObject[key] = value.getTime();
      return;
    }

    if (isostring(value)) {
      segmentObject[key] = new Date(value).getTime();
      return
    }

    //check inside nested objects as well
    if (is.object(value)) {
      //recurse
      return segmentObject[key] = unixifyAllDates(segmentObject[key])
    }
