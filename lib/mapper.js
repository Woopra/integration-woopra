...
function cookie(message){
  var woopraCookie;
  //use try/catch because property is 5 deep
  try{
    woopraCookie = message.options().context.integrations.woopra.cookie;
  }
  catch(e) {
    //Should be "cannot read property X of undefined."
    //Woopra cookie not provided
    //Do Nothing => allow to create cookie below:
  }

  if (is.defined(woopraCookie)) {
    return woopraCookie;
  }
  else {
    return crypto
